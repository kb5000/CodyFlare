...
void tts(void* dds) {
	...
	Color c;
	//expand
	//dfh->tMax = (dfh->tMax > 9 ? 9 : dfh->tMax + 0.1);
...
void test_of_function() {
	...
	*dfh = create_function_holder(calc_det, new_pos(5, 3.5), new_pos(-0.0, -0), 0.9, 0, 14, 0.1, 0, color_by_name("Black"), 1, 55.9);
